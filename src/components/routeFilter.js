...
const RouteFilter = props => (
  <div
    ...
    className={classNames(
      ...
      {
        [styles.filterContainerHideFilter]:
          props.isFullScreen && !props.showFilter
      }
    )}
  >
    {props.isFullScreen ? (
    ...
    <div
      className={props.isFullScreen && !props.showFilter ? styles.hidden : ""}
    >
      {Object.values(groupBy(props.routes, "dateBegin")).map(
        (routeDate, dateIndex) => (
          <div key={`routeFilterDateGroup${dateIndex}`}>
            <p className={styles.dateLabel}>
              {parseDate(routeDate[0].dateBegin)} -{" "}
              {parseDate(routeDate[0].dateEnd)}
            </p>
            <div className={styles.filterItemsWrapper}>
              {routeDate.map((route, routeIndex) => (
                <RouteFilterItem
                  key={`routeFilterItem_${dateIndex * routeDate.length +
                    routeIndex}`}
